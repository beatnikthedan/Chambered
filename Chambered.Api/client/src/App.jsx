...
export default function App() {
  ...
  useEffect(() => {
    const handleOutsideClick = (e) => {
      ...
      if (arsenalDropdownRef.current && !arsenalDropdownRef.current.contains(e.target)) {
        setShowArsenalDropdown(false)
      }
    }
  ...
  return (
    <div className={`app-container ${store.isAuthenticated ? 'logged-in' : ''}`}>
      {/* Collapsible Sidebar */}
      {store.isAuthenticated && (
        <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
          <div className="sidebar-header">
            <span className="logo-emoji">🔥</span>
            {!isSidebarCollapsed && <span className="logo-text">CHAMBERED</span>}
            <button 
          ...
          {store.isAuthenticated && !isSidebarCollapsed && (
            <div className="arsenal-selector-container" ref={arsenalDropdownRef}>
              <button 
              ...
              {showArsenalDropdown && (
                <div className="arsenal-dropdown-popover" onClick={(e) => e.stopPropagation()}>
          ...
          <nav className="sidebar-nav">
            ...
            <Link 
              ...
              title="Dashboard"
            >
              <span className="nav-text">{!isSidebarCollapsed ? 'Dashboard' : ''}</span>
            </Link>
            ...
            <div className="nav-group">
              <Link 
                ...
                title="Armory"
              >
                <span className="nav-text">{!isSidebarCollapsed ? 'Armory' : ''}</span>
              </Link>
              {!isSidebarCollapsed && location.pathname.startsWith('/armory') && (
                <div className="nav-sub-items">
                  <div className="sub-nav-item disabled" title="Maintenance Log Coming Soon">
            ...
            <div className="nav-group">
              <Link 
                ...
                title="Bench"
              >
                <span className="nav-text">{!isSidebarCollapsed ? 'Bench' : ''}</span>
              </Link>
              {!isSidebarCollapsed && location.pathname.startsWith('/bench') && (
                <div className="nav-sub-items">
                  <Link 
                    to="/bench/load-data" 
            ...
            <div className="nav-group">
              <Link 
                ...
                title="Range"
              >
                <span className="nav-text">{!isSidebarCollapsed ? 'Range' : ''}</span>
              </Link>
              {!isSidebarCollapsed && location.pathname.startsWith('/range') && (
                <div className="nav-sub-items">
                  <Link 
                    to="/range/trips" 
            ...
            <div className="nav-group">
              <Link 
                ...
                title="Vaults"
              >
                <span className="nav-text">{!isSidebarCollapsed ? 'Vaults' : ''}</span>
              </Link>
              {!isSidebarCollapsed && location.pathname.startsWith('/vaults') && (
                <div className="nav-sub-items">
                  <Link 
                    to="/vaults/locations" 
            ...
            {!isSidebarCollapsed ? (
              <div className="version-info">
                <div className="version-row">
                  ...
                  <div className="api-signal" title="Chambered API Connected" style={{ marginLeft: '8px' }}>
              ...
              <div className="version-info-collapsed" title="v1.0.0 (Docker) - Up to date">
                <span className="status-dot green"></span>
              </div>
            )}
          </div>
          ...
          <header className="top-header">
            ...
            {store.user && (
              <div className="header-right">
                <Link to="/settings" className="header-action-btn" title="Settings Menu">
                  ...
                  {showProfileMenu && (
                    <div className="profile-dropdown-menu" onClick={(e) => e.stopPropagation()}>
                      <div className="dropdown-header-sec">
                        <div className="header-username">{store.user.username}</div>
                        {store.user.email && <div className="header-email">{store.user.email}</div>}
                        {store.user.roles && store.user.roles.length > 0 && (
                          <div className="header-role">{store.user.roles[0]}</div>
                        )}
                      </div>
                      ...
                      <button className="dropdown-option-btn logout-option" onClick={handleLogout}>
                        Sign Out
                      </button>
                    </div>
                  )}
        ...
        <div className="content-wrapper">
          {store.loading ? (
            ...
            <Routes>
              ...
              {/* Bench routes */}
              <Route path="/bench/load-data" element={<ProtectedRoute><PlaceholderView /></ProtectedRoute>} />
              <Route path="/bench/components" element={<ProtectedRoute><PlaceholderView /></ProtectedRoute>} />
              
              {/* Range routes */}
              <Route path="/range/trips" element={<ProtectedRoute><PlaceholderView /></ProtectedRoute>} />
              <Route path="/range/targets" element={<ProtectedRoute><PlaceholderView /></ProtectedRoute>} />
        ...
        <div className="modal-overlay" onClick={() => setShowAccountModal(false)}>
          <div className="account-modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header-row">
              ...
              <button className="close-modal-btn" onClick={() => setShowAccountModal(false)}>×</button>
            </div>
            ...
            <div className="account-modal-content">
              <div className="profile-preview-row">
                {store.user.gravatarUrl ? (
                  ...
                  <div className="large-modal-avatar user-initials-avatar" style={{ fontSize: '24px' }}>
                    {store.user.username[0].toUpperCase()}
                  </div>
                )}
                ...
                {store.user.roles && store.user.roles.length > 0 && (
                  <div className="detail-item">
                    ...
                    <span className="item-val">
                      <span className="badge badge-danger">{store.user.roles[0]}</span>
                    </span>
                  </div>
                )}
              </div>
