      ...
      if (mode === 'setup') {
        ...
        try {
          ...
          if (res.ok) {
            ...
            throw new Error(txt || 'Registration is restricted by administrators.')
          }
        } catch (err) {
          throw new Error(err.message || 'Self-registration is restricted. Please contact your administrator.')
        }
      }
    } catch (err) {
      setErrorMessage(err.message || 'Authentication or profile creation failed.')
    } finally {
      ...
      <div className="login-card">
        ...
        <div className="brand">
          ...
          {mode === 'setup' ? (
            ...
            <>
              <h1 className="brand-name">CHAMBERED</h1>
              <p className="brand-subtitle">Precision armory & ammunition server</p>
            </>
          )}
        </div>
        ...
        {mode === 'setup' && (
          <div className="setup-banner">
            No profiles found. This first account becomes <strong>System Owner</strong> with full administrative rights.
          </div>
        )}
        ...
        {localLoginEnabled ? (
          <form onSubmit={handleSubmit} className="login-form">
            <div className="form-group">
              ...
              <input 
                ...
                placeholder={mode === 'setup' ? 'e.g. administrator' : 'administrator'} 
                required 
            ...
            {(mode === 'setup' || mode === 'register') && (
              <div className="form-group">
                <label htmlFor="email">Email Address {mode === 'setup' && '· optional'}</label>
                <input 
                  type="email" 
                  id="email" 
                  value={email}
            ...
            {(mode === 'setup' || mode === 'register') && (
              <div className="password-strength">
                <div className="password-strength-bar">
                  <div className={`strength-segment ${pwdStrength.score >= 1 ? `active-${pwdStrength.className}` : ''}`} />
                  <div className={`strength-segment ${pwdStrength.score >= 2 ? `active-${pwdStrength.className}` : ''}`} />
                  <div className={`strength-segment ${pwdStrength.score >= 3 ? `active-${pwdStrength.className}` : ''}`} />
                  <div className={`strength-segment ${pwdStrength.score >= 4 ? `active-${pwdStrength.className}` : ''}`} />
                </div>
          ...
          <div className="setup-banner" style={{ margin: '0 0 20px 0', border: '1px dashed rgba(255, 255, 255, 0.1)' }}>
            Local authentication is disabled. Please authenticate using OIDC Single Sign-On below.
          </div>
        )}
        ...
        {mode === 'login' && oidcProviders.length > 0 && (
          <>
            ...
            <div className="sso-actions" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        {store.isInitialized && (
          <div className="auth-toggle-container">
            {mode === 'login' ? (
              <>
                New user?
                <button type="button" onClick={() => { setErrorMessage(''); setMode('register'); }} className="auth-toggle-link">
                  Create an account
                </button>
              </>
            ) : (
              <>
                ...
                <button type="button" onClick={() => { setErrorMessage(''); setMode('login'); }} className="auth-toggle-link">
                  Sign in
                </button>
              </>
            )}
